...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['deactivate_product']) || isset($_POST['activate_product'])) {
        ...
        if (!$conn->query("CALL sp_update_product_status({$product_id}, '{$status}')")) {
            ...
            $u->bind_param("si", $status, $product_id); $u->execute(); $u->close();
        } else { flush_results($conn); }
    ...
    if (isset($_POST['approve_order']) || isset($_POST['reject_order']) || isset($_POST['complete_order'])) {
        ...
        if (!$conn->query("CALL sp_update_sale_status({$order_id}, '{$status}')")) {
            $u = $conn->prepare("UPDATE sales SET status=? WHERE id=?"); $u->bind_param("si", $status, $order_id); $u->execute(); $u->close();
        } else { flush_results($conn); }
    ...
    if (isset($_POST['deactivate_user']) || isset($_POST['activate_user'])) {
        ...
        if (!$conn->query("CALL sp_update_user_status({$user_id}, '{$status}')")) {
            $u = $conn->prepare("UPDATE users SET status=? WHERE id=?"); $u->bind_param("si", $status, $user_id); $u->execute(); $u->close();
        } else { flush_results($conn); }
    ...
    if (isset($_POST['create_user'])) {
        $name = $_POST['name'] ?? ''; $username = $_POST['username'] ?? ''; $password = password_hash($_POST['password'] ?? '', PASSWORD_DEFAULT); $role = $_POST['role'] ?? 'cashier';
        if (!$conn->query("CALL sp_create_user(" . $conn->real_escape_string("'{$name}'") . ", " . $conn->real_escape_string("'{$username}'") . ", " . $conn->real_escape_string("'{$password}'") . ", " . $conn->real_escape_string("'{$role}'") . ")")) {
            ...
            $u->bind_param("ssss", $name, $username, $password, $role); $u->execute(); $u->close();
        } else { flush_results($conn); }
    ...
    if (isset($_POST['edit_user'])) {
        $edit_user_id = (int)($_POST['edit_user_id'] ?? 0); $edit_name = $_POST['edit_name'] ?? ''; $edit_username = $_POST['edit_username'] ?? ''; $edit_role = $_POST['edit_role'] ?? 'cashier';
        if (!$conn->query("CALL sp_edit_user({$edit_user_id}, " . $conn->real_escape_string("'{$edit_name}'") . ", " . $conn->real_escape_string("'{$edit_username}'") . ", " . $conn->real_escape_string("'{$edit_role}'") . ")")) {
            $u = $conn->prepare("UPDATE users SET name=?, username=?, role=? WHERE id=?"); $u->bind_param("sssi", $edit_name, $edit_username, $edit_role, $edit_user_id); $u->execute(); $u->close();
        } else { flush_results($conn); }
...
if ($res && $res instanceof mysqli_result) {
    ...
    $res->free(); flush_results($conn);
} else {
    $stmt = $conn->prepare("SELECT COUNT(*) AS order_count FROM sales WHERE DATE(created_at) = ?");
    $stmt->bind_param("s", $current_date); $stmt->execute(); $stmt->bind_result($order_count_today); $stmt->fetch(); $stmt->close();
}

$threshold = 10; $is_busy_day = ($order_count_today > $threshold);
...
<head>
    ...
    <style>
        ...
        body{display:flex;min-height:100vh;margin:0;font-family:"Poppins",sans-serif;background:#f7f8fa;color:#222;}
        .sidebar{width:260px;flex-shrink:0;background:linear-gradient(180deg,#d9ed42 0%,#d39e2a 60%,#e0d979ff 100%);padding:25px 20px;position:fixed;top:0;bottom:0;left:0;box-shadow:4px 0 20px rgba(0,0,0,.1);border-top-right-radius:20px;}
        .main-content{flex-grow:1;margin-left:260px;padding:40px 50px;background:#fafafa;min-height:100vh;box-sizing:border-box;overflow-y:auto;}
        .sidebar .logo{display:flex;align-items:center;gap:10px;font-size:26px;font-weight:800;margin-bottom:8px;}
        ...
        .nav-link{display:flex;align-items:center;gap:8px;text-decoration:none;color:#000;font-weight:600;padding:12px 18px;border-radius:10px;transition:all .15s ease;}
        .nav-link.active{background:rgba(0,0,0,.08);transform:translateX(4px);}
        .btn-logout{margin-top:auto;width:100%;background:linear-gradient(135deg,#000 0%,#222 100%);color:#fff;border-radius:10px;padding:12px 0;border:none;font-weight:600;}
        .dashboard-cards{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:20px;margin-bottom:20px;}
        .card-brief{background:#fff;border-radius:12px;padding:18px;box-shadow:0 3px 12px rgba(0,0,0,.06);text-align:center;}
        .chart-card{background:#fff;padding:20px;border-radius:12px;box-shadow:0 3px 10px rgba(0,0,0,.06);}
        .content-section{display:none;}
        @media(max-width:800px){.sidebar{position:relative;width:100%;border-radius:0;} .main-content{margin-left:0;padding:20px;}}
    </style>
</head>
<body>
    <div class="sidebar">
    ...
    <div class="main-content container-fluid">
        ...
        <div id="dashboard" class="content-section" style="display:block;">
            ...
            <div class="dashboard-cards mb-3">
                <div class="card-brief"><h5>Total Products</h5><p class="h4 mb-0"><?php echo number_format($analytics['total_products'] ?? 0); ?></p></div>
                <div class="card-brief"><h5>Total Users</h5><p class="h4 mb-0"><?php echo number_format($analytics['total_users'] ?? 0); ?></p></div>
                <div class="card-brief"><h5>Total Orders</h5><p class="h4 mb-0"><?php echo number_format($analytics['total_orders'] ?? 0); ?></p></div>
                <div class="card-brief"><h5>Total Sales</h5><p class="h4 mb-0">₱<?php echo number_format((float)($analytics['total_sales'] ?? 0), 2); ?></p></div>
            </div>
            ...
            <div class="table-responsive">
                <table class="table table-striped align-middle">
                    <thead>
                        <tr><th>Order ID</th><th>Supplier</th><th>Status</th><th>Total Amount</th><th>Created At</th></tr>
                    </thead>
                    <tbody>
        ...
        <div id="reports" class="content-section">
            ...
            <div class="table-responsive mb-4">
                <table class="table table-bordered">
                    <thead><tr><th>Action ID</th><th>User</th><th>Action Type</th><th>Details</th><th>Action Time</th></tr></thead>
                    <tbody>
            ...
            <div class="table-responsive">
                <table class="table table-bordered">
                    <thead><tr><th>Message ID</th><th>Message</th><th>Created At</th></tr></thead>
                    <tbody>
        ...
        <div id="products" class="content-section">
            ...
            <div class="table-responsive">
                <table class="table table-bordered align-middle">
                    <thead><tr><th>Name</th><th>Description</th><th>Price</th><th>Stock</th><th>Status</th><th>Action</th></tr></thead>
                    <tbody>
                        <?php if ($products_result && $products_result instanceof mysqli_result) {
                            while ($product = $products_result->fetch_assoc()) { ?>
                                <tr>
                                    ...
                                    <td><?php echo (int)$product['stock']; if ((int)$product['stock'] < 5) echo ' <span class="badge bg-danger">Low Stock</span>'; ?></td>
        ...
        <div id="orders" class="content-section">
            ...
            <div class="table-responsive">
                <table class="table table-bordered align-middle">
                    <thead><tr><th>Order ID</th><th>Supplier</th><th>Total Amount</th><th>Status</th><th>Date</th><th>Action</th></tr></thead>
                    <tbody>
                            ...
                            while ($order = $orders_result->fetch_assoc()) { ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <?php } else { echo '<span class="text-muted">'.htmlspecialchars(ucfirst($order['status'])).'</span>'; } ?>
                                    </td>
                                </tr>
                                    ...
                                    <tr><td colspan="6">
                                        ...
                                        <div class="table-responsive mt-2">
                                            <table class="table table-sm table-bordered">
                                                <thead><tr><th>Product Name</th><th>Quantity</th><th>Price</th><th>Total Price</th></tr></thead>
                                                <tbody>
            ...
            <form method="POST" enctype="multipart/form-data" class="mb-4">
                ...
                <div class="row">
                    <div class="col-md-4 mb-3"><label class="form-label">Role</label>
                        <select name="role" class="form-select" required><option value="cashier">Cashier</option><option value="encoder">Encoder</option><option value="supervisor">Supervisor</option></select>
                    </div>
